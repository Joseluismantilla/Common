...
_puppet()
{
    ...
    case ${words[0]} in
        ...
        puppet)
            case ${words[1]} in
               agent|apply|ca|catalog|cert|certificate|config|describe|device|doc|facts|file|filebucket|help|inspect|instrumentation|key|kick|man|master|module|node|parser|plugin|queue|report|resource|secret|status)
                   command=${words[1]}
                   ;;
               *.pp|*.rb|-*)
                   command=apply
                   ;;
               *)
                  COMPREPLY=( $( compgen -W 'agent apply ca catalog cert certificate config describe device doc facts file
                      filebucket help inspect instrumentation key kick man master module node parser plugin queue report resource secret status' -- "$cur" ) )
                  return 0
                  ;;
            esac
    esac
...
complete -F _puppet puppet #puppetmasterd puppetd puppetca ralsh puppetrun puppetqd filebucket puppetdoc puppet